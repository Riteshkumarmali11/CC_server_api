...
const tcQuList=require("./service/TcqueueList");
const newjointOrder=require("./service/newjointOrderService");
const rfid=require("./service/rfid_insert");
const createvillage=require("./service/createvillage");
const farmerList = require('./service/getfarmerList');
...
app.post('/createvillage', async (req, res, next) => {
...
app.post('/holist', async (req, res, next) => {
         HoListService.hoList(req, res)
        });
...
app.post('/farmerList', async (req, res, next) => {
    farmerList.farmerData(req, res);
});

app.post('/farmerListData', async (req, res, next) => {
    getFarmerDetails.farmerInfoData(req, res);
});

app.post('/plantationDetails', async (req, res, next) => {
    getPlantationDetails.plantationDetail(req, res)
});

app.post('/farmerData', async (req, res, next) => {
    displayFarmer.farmerinfo(req, res);
});
...
app.post('/rfid/insert', async (req, res, next) => {
    rfid.insert_rfid(req, res);
});


//-------------------------------------duplicate api for internal network issue -----------------------

app.post('/api/rfid/insert', async (req, res, next) => {
    rfid.insert_rfid(req, res);
});
...
app.get('/api/rfid/tcq/:id', async (req, res, next) => {
    tcQuList.tcqList(req, res)
});

app.get('/rfid/tcq/:id', async (req, res, next) => {
    tcQuList.tcqList(req, res)
});


app.post('/new/joint/order', async (req, res, next) => {
    newjointOrder.newjointOrder(req, res);
});
app.post('/api/new/joint/order', async (req, res, next) => {
    newjointOrder.newjointOrder(req, res);
});


app.listen(port, () => { console.log("Listening to the server on http://122.169.40.197:5000") });
